  ...
  async parseRow(row, prisma) {
    ...
    return {
      data: {
        ...
        externalUid: row.externalUid?.trim() || null,
        tagline: row.tagline?.trim() || null,
        officialLanguage: row.officialLanguage?.trim() || null,
        capitalCity: row.capitalCity?.trim() || null,
        currencyCode: row.currencyCode?.trim().toUpperCase() || null,
        currencyName: row.currencyName?.trim() || null,
