...
export function CoursePageView({ course, country }: { course: CourseDetail; country?: string }) {
  ...
  return <main>
    ...
    <section className="detail-content shell"><div className="detail-main">
      <section className="editorial-section" aria-labelledby="course-overview"><p className="eyebrow">Course overview</p><h2 id="course-overview">What you will explore</h2><RichText value={resolveContentVariables('course', course.overview ?? course.shortDescription ?? 'Course overview is not available yet.', course as unknown as Record<string, unknown>)} />{course.careerSummary ? <p>{course.careerSummary}</p> : null}</section>
