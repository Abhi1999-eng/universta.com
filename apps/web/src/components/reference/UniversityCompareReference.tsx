...
export function UniversityCompareReference(
  props: UniversityCompareReferenceProps,
) {
  ...
  function go(next: string[]) {
    ...
    window.localStorage.setItem(
      "universta:compare:universities",
      JSON.stringify(unique),
    );
    router.push(
      unique.length ? `${pathname}?items=${unique.join(",")}` : pathname,
    );
  }
  ...
  const rows: Array<[string, (item: CompareUniversity) => React.ReactNode]> = [
    ...
    [
      "Institution type",
      (item) => (item.institutionType ? humanise(item.institutionType) : "—"),
    ],
    [
      ...
      (item) =>
        item.offerings ? (
          <Link
            href={`/universities/${item.slug}/courses`}
    ...
    [
      "Campuses",
      (item) => (item.campuses ? <b>{formatNumber(item.campuses)}</b> : "—"),
    ],
    [
      "Accreditations",
      (item) =>
        item.accreditations.length ? item.accreditations.join(", ") : "—",
    ],
    [
      "Record verified",
      (item) =>
        item.verifiedAt ? formatDate(item.verifiedAt) : "Not verified",
    ],
    ...
    <div className="cref cref-dest">
      ...
      <section className="hero">
        <div className="wrap hero-in">
          ...
          <p className="lead">
            Line up to {MAX} published institutions side by side on the fields
            the catalogue records. Nothing here is scored or ranked.
          </p>
        </div>
      </section>
      ...
      <section className="sec wrap" style={{ paddingTop: 8 }}>
        <div className="cmp-picker">
          <div className="cmp-search">
            <label htmlFor="compare-search-universities">
              Search published universities
            </label>
            <input
          ...
          {available.length ? (
            <div
              className="cmp-options"
              role="list"
              aria-label="Available universities"
            >
            ...
            <p className="cmp-note">
              {options.length
                ? "No published university matches that search."
                : "No published universities yet."}
            </p>
          )}
        ...
        {props.invalid.length ? (
          <p className="disclaimer" style={{ marginBottom: 18 }}>
            Not published, so left out of the comparison:{" "}
            {props.invalid.join(", ")}.
          </p>
        ) : null}
        ...
        {items.length === 0 ? (
          <div className="cref-empty" data-testid="compare-empty">
            ...
            <p>
              Add up to {MAX} universities above to compare them side by side.
            </p>
      ...
      <section className="sec wrap">
        <div className="cta2">
          ...
          <p>
            A counsellor can weigh these institutions against your marks, budget
            and target intake.
          </p>
