...
export function ComparisonSelector({
  ...
  const [selected, setSelected] = useState(() => {
    if (initial.length || typeof window === "undefined")
      return initial.slice(0, 3);
    try {
      const candidates = JSON.parse(
        window.localStorage.getItem(`universta:compare:${type}`) ?? "[]",
      ) as unknown;
      return Array.isArray(candidates)
        ? candidates
            .filter(
              (item): item is string =>
                typeof item === "string" &&
                options.some((option) => option.slug === item),
            )
            .slice(0, 3)
        : [];
    } catch {
  ...
  const compare = () => {
    ...
    window.localStorage.setItem(
      `universta:compare:${type}`,
      JSON.stringify(selected),
    );
    router.push(
      `/compare/${type}?items=${encodeURIComponent(selected.join(","))}`,
    );
  };
  return (
    <div className="comparison-selector">
      ...
      >
        {available.slice(0, 8).map((option) => (
          <button
            ...
            onClick={() => {
              setSelected((items) => [...items, option.slug]);
              setQuery("");
            }}
          >
      ...
      <div
        ...
        aria-label="Selected comparison items"
      >
        {selected.map((slug) => {
          const option = options.find((item) => item.slug === slug);
          return (
            <span key={slug}>
              {option?.name ?? option?.title ?? slug}
              <button
                type="button"
                aria-label={`Remove ${option?.name ?? option?.title ?? slug}`}
                onClick={() =>
                  setSelected((items) => items.filter((item) => item !== slug))
                }
              >
                ×
              </button>
