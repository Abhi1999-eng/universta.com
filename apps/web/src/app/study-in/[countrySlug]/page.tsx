...
export default async function CountryDetailPage({ params }: Props) { const slug = (await params).countrySlug; const page = await load(slug); if (!page) notFound(); const cities = await loadCities(slug); const jsonLd = { '@context': 'https://schema.org', '@type': 'Place', name: page.country.name, description: page.country.shortDescription, url: `/study-in-${page.country.slug}` }; return <><ApprovedCountryDetail page={page} cities={cities} /><script type="application/ld+json">{JSON.stringify(jsonLd).replace(/</g, '\\u003c')}</script></>; }
