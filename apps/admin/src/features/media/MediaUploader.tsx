...
/** Shared "Upload new" panel. Used inside the catalog MediaPickerDialog and
 * (as the single-file path) inside the Media Library page, so upload
 * validation/progress/cancel behaviour only lives in one place. */
export function MediaUploader({
  ...
  return (
    <form onSubmit={(event) => void submit(event)} className="space-y-4">
      ...
      >
        {preview ? (
          <img src={preview} alt="Selected file preview" className="max-h-32 rounded-lg object-contain" />
        ) : (
        ...
        {file ? <p className="truncate text-xs font-semibold text-[#48505F]">{file.name}</p> : null}
      </div>
