      ...
      {creating ? (
        <form onSubmit={(event) => void create(event)} className="mt-4 space-y-3 rounded-xl border border-[#E8ECF3] p-4">
          <div className="block text-sm font-semibold">
            <FieldLabel label="Name" htmlFor="lookup-name" required helpKey={`${kind === 'intake' ? 'intakes' : 'scholarship-providers'}.name`} />
            <input id="lookup-name" required className="mt-1 w-full rounded-lg border border-[#D9E0EA] px-3 py-2 font-normal" value={name} onChange={(event) => setName(event.target.value)} />
          </div>
          {kind === 'intake' ? (
            <div className="block text-sm font-semibold">
              <FieldLabel label="Month number (1-12, optional)" htmlFor="lookup-month" helpKey="intakes.monthNumber" />
              <input id="lookup-month" type="number" min="1" max="12" className="mt-1 w-full rounded-lg border border-[#D9E0EA] px-3 py-2 font-normal" value={monthNumber} onChange={(event) => setMonthNumber(event.target.value)} />
            </div>
          ) : (
            <div className="block text-sm font-semibold">
              <FieldLabel label="Website URL (optional)" htmlFor="lookup-website" help={commonFieldHelp.websiteUrl} />
              <input id="lookup-website" className="mt-1 w-full rounded-lg border border-[#D9E0EA] px-3 py-2 font-normal" value={websiteUrl} onChange={(event) => setWebsiteUrl(event.target.value)} />
            </div>
          )}
