...
import { deleteCountry, listContinents, listCountries, publishCountry, unpublishCountry } from './catalog-client';
import type { CatalogMutationError, ContinentRecord, CountryRecord, PageMeta } from './catalog.types';
...
export function CountriesPage() {
  ...
  const load = useCallback((signal?: AbortSignal) => {
    ...
    return listCountries({ q, continentId, status, featured: featured === '' ? undefined : featured === 'true', page, limit: 12 }, signal).then((result) => { setRows(result.data); setMeta(result.meta); setLoading(false); }).catch((cause: unknown) => { if (cause instanceof DOMException && cause.name === 'AbortError') return; setLoading(false); setError(cause instanceof Error ? cause.message : 'Unable to load countries'); });
  }, [continentId, featured, page, q, status]);
  ...
  function resetFilters() { setQ(''); setContinentId(''); setStatus(''); setFeatured(''); setPage(1); }
