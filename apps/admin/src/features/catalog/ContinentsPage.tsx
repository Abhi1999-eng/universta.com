...
export function ContinentsPage() {
  ...
  return (
    <section aria-labelledby="continents-heading" className="mx-auto max-w-[1180px]">
      ...
      {form ? <CatalogDialog title={form.record ? 'Edit continent' : 'Create continent'} description="Use the existing continent fields. Slugs are stable identifiers for public region URLs." onClose={() => setForm(null)}><form onSubmit={(event) => void save(event)} className="space-y-4"><Field label="Name" value={form.values.name} onChange={(value) => updateField('name', value)} required /><Field label="Slug" value={form.values.slug} onChange={(value) => updateField('slug', value)} help="Lowercase letters, numbers and hyphens." required /><div className="grid gap-4 sm:grid-cols-2"><Field label="Code" value={form.values.code} onChange={(value) => updateField('code', value)} /><Field label="Display order" value={form.values.displayOrder} onChange={(value) => updateField('displayOrder', value)} type="number" /></div><Field label="Description" value={form.values.description} onChange={(value) => updateField('description', value)} textarea /><label className="block text-sm font-semibold">Status<select value={form.values.status} onChange={(event) => updateField('status', event.target.value)} className="mt-2 w-full rounded-xl border border-[#D9E0EA] px-4 py-3 font-normal outline-none focus:border-[#1657CF]"><option value="ACTIVE">Active</option><option value="INACTIVE">Inactive</option></select></label>{error ? <p role="alert" className="text-sm text-[#B42318]">{error}</p> : null}<div className="flex justify-end gap-3 pt-3"><button type="button" onClick={() => setForm(null)} className="rounded-xl border border-[#D9E0EA] px-4 py-3 text-sm font-semibold">Cancel</button><button type="submit" className="rounded-xl bg-[#1657CF] px-5 py-3 text-sm font-semibold text-white">Save continent</button></div></form></CatalogDialog> : null}
...
function Field({ label, value, onChange, help, required, type = 'text', textarea = false }: { label: string; value: string; onChange: (value: string) => void; help?: string; required?: boolean; type?: string; textarea?: boolean }) {
  return <label className="block text-sm font-semibold">{label}{required ? ' *' : ''}{textarea ? <textarea required={required} value={value} onChange={(event) => onChange(event.target.value)} rows={3} className="mt-2 w-full rounded-xl border border-[#D9E0EA] px-4 py-3 font-normal outline-none focus:border-[#1657CF]" /> : <input required={required} type={type} value={value} onChange={(event) => onChange(event.target.value)} className="mt-2 w-full rounded-xl border border-[#D9E0EA] px-4 py-3 font-normal outline-none focus:border-[#1657CF]" />}{help ? <span className="mt-1 block text-xs font-normal text-[#828B9B]">{help}</span> : null}</label>;
}
