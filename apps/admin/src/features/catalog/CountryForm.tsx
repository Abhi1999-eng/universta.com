...
import { createConsultantCard, createContinent, createCountry, createCountryFaq, createEditorialSection, deleteConsultantCard, deleteCountryFaq, deleteCountrySeo, deleteEditorialSection, getCountry, getCountryCurationOptions, getCountryEditorial, listContinents, listEditorialMedia, publishCountry, saveCountrySeo, unpublishCountry, updateConsultantCard, updateCountry, updateCountryFaq, updateEditorialSection } from './catalog-client';
import type { CatalogMutationError, ContinentRecord, CountryEditorialBundle, CountryCurationOptions, CountryRecord, EditorialMedia, EditorialSeo } from './catalog.types';
import { MediaPickerDialog } from './editorial/MediaPickerDialog';
import { TypedBodyEditor } from './editorial/TypedBodyEditor';
import { SECTION_KEYS, SECTION_TYPES, blankSection, bodyForApi, draftFromSection, type SectionDraft, type SectionType } from './editorial/editor-types';
...
export function CountryForm({ countryId }: { countryId?: string }) {
  ...
  const [configuration, setConfiguration] = useState<CountryConfiguration>(blankConfiguration);
  const [curationOptions, setCurationOptions] = useState<CountryCurationOptions | null>(null);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const [continentResult, mediaResult] = await Promise.all([listContinents({ limit: 100 }), listEditorialMedia({ limit: 50 })]);
        if (!active) return;
        setContinents(continentResult.data);
        setMedia(mediaResult.data);
        if (!countryId) return;
        const [countryResult, editorialResult, curationResult] = await Promise.all([getCountry(countryId), getCountryEditorial(countryId), getCountryCurationOptions(countryId)]);
        if (!active) return;
        ...
        setConfiguration({
          featureCodes: country.configuration?.features.map((feature) => feature.code) ?? [],
          acceptedTests: country.configuration?.acceptedTests ?? [],
          intakeMonths: country.configuration?.intakeMonths ?? [],
          postStudyWorkPermitMonths: country.configuration?.postStudyWorkPermitMonths === null || country.configuration?.postStudyWorkPermitMonths === undefined ? '' : String(country.configuration.postStudyWorkPermitMonths),
        ...
        if (active) setError(cause instanceof Error ? cause.message : 'Unable to load country editor');
      } finally {
  ...
  const toggleConfiguration = (key: 'featureCodes' | 'acceptedTests' | 'intakeMonths' | 'popularUniversityIds' | 'popularCourseIds', value: string | number) => {
    setConfiguration((current) => {
      const values = current[key] as Array<string | number>;
      const included = values.includes(value);
      return {
        ...current,
        [key]: included ? values.filter((item) => item !== value) : [...values, value],
  ...
  function validate() {
    ...
    if (!core.shortDescription.trim()) next.push('Short description is required.');
    activeSections.forEach((row, index) => {
      if (!row.heading.trim()) next.push(`Content section ${index + 1}: heading is required.`);
      if (row.ctaUrl && !/^\/(?!\/)|^#[a-zA-Z0-9_-]+$|^https:\/\//.test(row.ctaUrl)) next.push(`Content section ${index + 1}: CTA URL is invalid.`);
    });
    activeFaqs.forEach((row, index) => {
      if (!row.question.trim() || !row.answer.trim()) next.push(`FAQ ${index + 1}: question and answer are required.`);
    });
    activeCards.forEach((row, index) => {
      if (!row.title.trim() || !row.shortDescription.trim()) next.push(`Guidance card ${index + 1}: title and short description are required.`);
    });
  ...
  async function syncEditorial(id: string) {
    ...
    for (const row of activeSections) {
      ...
      const result = row.id ? await updateEditorialSection(id, row.id, payload) : await createEditorialSection(id, payload);
      nextSections.push({
    ...
    for (const row of activeFaqs) {
      ...
      const result = row.id ? await updateCountryFaq(id, row.id, payload) : await createCountryFaq(id, payload);
      nextFaqs.push({
        id: result.data.id,
        updatedAt: result.data.updatedAt,
    ...
    for (const row of activeCards) {
      ...
      const result = row.id ? await updateConsultantCard(id, row.id, payload) : await createConsultantCard(id, payload);
    ...
    if (hasSeo(seo)) {
      const result = await saveCountrySeo(id, {
        ...seoPayload(seo),
        ...(existingSeo?.schemaJson ? { schemaJson: existingSeo.schemaJson } : {}),
        ...(existingSeo?.hreflangJson ? { hreflangJson: existingSeo.hreflangJson } : {}),
      });
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const payload = {
        ...
        postStudyWorkPermitMonths: configuration.postStudyWorkPermitMonths === '' ? undefined : Number(configuration.postStudyWorkPermitMonths),
      ...
      setError(typed.message ?? (cause instanceof Error ? cause.message : 'Unable to save country'));
    } finally {
      setSaving(false);
  ...
  if (loading)
    return (
      <section className="mx-auto max-w-[1100px] rounded-2xl border border-[#E8ECF3] bg-white p-8">
        <p className="text-sm text-[#667085]">Loading complete country editor…</p>
      </section>
    );

  return (
    <section className="mx-auto max-w-[1180px]" aria-labelledby="country-form-heading">
      <Link href="/countries" className="text-sm font-semibold text-[#1657CF]">
        ← Countries
      </Link>
      <div className="mt-8 flex flex-wrap items-end justify-between gap-4">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.16em] text-[#828B9B]">Unified country editor</p>
          <h2 id="country-form-heading" className="mt-2 text-3xl font-semibold">
            {record ? 'Edit country' : 'Create country'}
          </h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-[#667085]">Editorial country information, configuration and curated relationships. University and offering facts are derived automatically.</p>
        </div>
      ...
      {error ? (
        <p role="alert" className="mt-5 rounded-xl border border-[#F2C5C5] bg-[#FFF7F7] px-4 py-3 text-sm font-semibold text-[#B42318]">
          {error}
        </p>
      ) : null}
      {issues.length ? (
        <div role="alert" className="mt-5 rounded-xl border border-[#F2C5C5] bg-[#FFF7F7] p-4 text-sm text-[#B42318]">
      ...
      <form onSubmit={submit} className="mt-8 space-y-6">
        <Card eyebrow="Country" title="Identity & listing" description="Core catalogue identity and public listing content. ISO and currency are filled from local canonical country metadata.">
          <div className="grid gap-5 sm:grid-cols-2">
            ...
            />
            <Input label="Country name" value={core.name} onChange={(value) => setCoreField('name', value)} />
            <Input label="Slug" value={core.slug} onChange={(value) => setCoreField('slug', value)} />
            <Input label="Display order" value={core.displayOrder} onChange={(value) => setCoreField('displayOrder', value)} type="number" />
            <Input label="Page heading" value={core.pageHeading} onChange={(value) => setCoreField('pageHeading', value)} span />
            <Input label="Short description" value={core.shortDescription} onChange={(value) => setCoreField('shortDescription', value)} textarea span rows={4} />
          </div>
          {record?.currency ? (
            <p className="mt-5 rounded-xl bg-[#F8FAFC] px-4 py-3 text-sm text-[#475467]">
              Derived identity: {record.iso2Code} / {record.iso3Code} · {record.currency.code} {record.currency.symbol ?? ''}
            </p>
          ) : null}
          {/* `isFeatured` remains stored for compatibility with legacy listing order, but has no Country Admin control or public rendering. */}
        </Card>
        <Card eyebrow="Configuration" title="Study destination setup" description="Only country-level editorial configuration belongs here. Tuition, rankings, statistics and admission scores are derived from published university and offering data.">
          <div className="space-y-6">
            ...
            />
            <CheckboxGroup title="Accepted English tests" options={testOptions.map((value) => ({ value, label: value }))} selected={configuration.acceptedTests} onToggle={(value) => toggleConfiguration('acceptedTests', value)} />
            <CheckboxGroup
              ...
              onToggle={(value) => toggleConfiguration('intakeMonths', Number(value))}
            />
            <Input
            ...
            <div className="grid gap-5 lg:grid-cols-2">
              <RelationPicker title="Popular Universities" description="Published universities in this country only." options={curationOptions?.universities ?? []} selected={configuration.popularUniversityIds} onToggle={(value) => toggleConfiguration('popularUniversityIds', value)} disabled={!record} />
              <RelationPicker title="Popular Courses" description="Published courses available in this country only." options={curationOptions?.courses ?? []} selected={configuration.popularCourseIds} onToggle={(value) => toggleConfiguration('popularCourseIds', value)} disabled={!record} />
            </div>
          </div>
        </Card>
        <Card eyebrow="Editorial" title="Content sections" description="All public country sections are edited here; no separate section save.">
          ...
          <div className="mt-5 space-y-5">{sections.length === 0 ? <Empty text="No editorial sections yet." /> : sections.map((row, index) => <CountrySection key={row.id ?? `section-${index}`} index={index} row={row} media={media} onChange={(patch) => updateSection(index, patch)} onRemove={() => removeSection(index)} />)}</div>
        </Card>
        <Card eyebrow="Questions" title="FAQs" description="FAQs are part of the country save flow.">
          <div className="flex justify-end">
            <button
              type="button"
              onClick={() => {
          ...
          <div className="mt-5 space-y-4">
            {faqs.length === 0 ? (
              ...
              faqs.map((row, index) => (
                <div key={row.id ?? `faq-${index}`} className="rounded-2xl border border-[#E8ECF3] bg-[#FBFCFE] p-5">
                  <div className="flex justify-between">
                    <h4 className="font-semibold">FAQ {index + 1}</h4>
                    <button type="button" onClick={() => removeFaq(index)} className="text-sm font-semibold text-[#B42318]">
                      Remove
                    </button>
                  </div>
                  <div className="mt-4 grid gap-4 sm:grid-cols-2">
                    <Input label="Question" value={row.question} onChange={(value) => updateFaq(index, { question: value })} span />
                    <Input label="Answer" value={row.answer} onChange={(value) => updateFaq(index, { answer: value })} textarea span rows={4} />
                    <Input label="Category" value={row.category} onChange={(value) => updateFaq(index, { category: value })} />
                    <Input label="Display order" value={row.displayOrder} onChange={(value) => updateFaq(index, { displayOrder: value })} type="number" />
                    <BooleanField label="Featured FAQ" checked={row.isFeatured} onChange={(checked) => updateFaq(index, { isFeatured: checked })} />
                  </div>
                </div>
              ))
            )}
        ...
        <Card eyebrow="Guidance" title="Consultant cards" description="Optional guidance cards are staged here and saved with the country.">
          <div className="flex justify-end">
            <button
              type="button"
              onClick={() => {
          ...
          <div className="mt-5 space-y-5">
            {cards.length === 0 ? (
              ...
              cards.map((row, index) => (
                <div key={row.id ?? `card-${index}`} className="rounded-2xl border border-[#E8ECF3] bg-[#FBFCFE] p-5">
                  <div className="flex justify-between">
                    <h4 className="font-semibold">Guidance card {index + 1}</h4>
                    <button type="button" onClick={() => removeCard(index)} className="text-sm font-semibold text-[#B42318]">
                      Remove
                    </button>
                  </div>
                  <div className="mt-4 grid gap-4 sm:grid-cols-2">
                    ...
                    />
                    <Input label="Slug" value={row.slug} onChange={(value) => updateCard(index, { slug: value })} />
                    <Input label="Short description" value={row.shortDescription} onChange={(value) => updateCard(index, { shortDescription: value })} textarea span />
                    <Input label="Overview" value={row.overview} onChange={(value) => updateCard(index, { overview: value })} textarea span />
                    <MediaPickerDialog label="Icon media" value={row.iconMediaId} media={media} onChange={(value) => updateCard(index, { iconMediaId: value })} />
                    <MediaPickerDialog label="Featured media" value={row.featuredMediaId} media={media} onChange={(value) => updateCard(index, { featuredMediaId: value })} />
                    <Input label="CTA label" value={row.ctaLabel} onChange={(value) => updateCard(index, { ctaLabel: value })} />
                    <Input label="CTA URL" value={row.ctaUrl} onChange={(value) => updateCard(index, { ctaUrl: value })} />
                    <Input label="Display order" value={row.displayOrder} onChange={(value) => updateCard(index, { displayOrder: value })} type="number" />
                    <BooleanField label="Free consultation" checked={row.isFreeConsultation} onChange={(checked) => updateCard(index, { isFreeConsultation: checked })} />
                    <BooleanField label="Featured card" checked={row.isFeatured} onChange={(checked) => updateCard(index, { isFeatured: checked })} />
                  </div>
                </div>
              ))
            )}
...
function Card({ eyebrow, title, description, children }: { eyebrow: string; title: string; description: string; children: React.ReactNode }) {
  return (
    <fieldset className="rounded-2xl border border-[#E8ECF3] bg-white p-6 sm:p-8">
      ...
      <p className="text-xs font-bold uppercase tracking-[0.14em] text-[#1657CF]">{eyebrow}</p>
...
function Empty({ text }: { text: string }) {
  return <div className="rounded-xl bg-[#F8FAFC] p-5 text-sm text-[#667085]">{text}</div>;
}
function Input({ label, value, onChange, type = 'text', textarea = false, span = false, rows = 3 }: { label: string; value: string; onChange: (value: string) => void; type?: string; textarea?: boolean; span?: boolean; rows?: number }) {
  ...
  return (
    <div className={`text-sm font-semibold ${span ? 'sm:col-span-2' : ''}`}>
      ...
      {textarea ? <textarea id={id} rows={rows} className={input} value={value} onChange={(event) => onChange(event.target.value)} /> : <input id={id} type={type} className={input} value={value} onChange={(event) => onChange(event.target.value)} />}
    </div>
  );
}
  ...
  return (
    <div className="text-sm font-semibold">
      <div className="flex items-center justify-between gap-3">
        ...
        <button type="button" onClick={() => setOpen(true)} className="text-xs font-semibold text-[#1657CF] focus:outline-none focus:underline">
          + Add continent
        </button>
      </div>
        ...
        <CatalogDialog title="Add continent" description="This creates a catalogue continent straight away and selects it for this country." onClose={close}>
          ...
          >
            ...
            <div className="flex justify-end gap-3 pt-2">
              <button type="button" onClick={close} className="rounded-xl border border-[#D9E0EA] px-4 py-3 text-sm font-semibold">
                Cancel
              </button>
              <button type="button" onClick={() => void submit()} disabled={busy || !draft.name.trim()} className="rounded-xl bg-[#1657CF] px-5 py-3 text-sm font-semibold text-white disabled:opacity-40">
...
function CheckboxGroup({ title, options, selected, onToggle }: { title: string; options: Array<{ value: string | number; label: string }>; selected: string[]; onToggle: (value: string) => void }) {
  return (
    <fieldset>
      <legend className="text-sm font-semibold text-[#344054]">{title}</legend>
      <div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {options.map((option) => (
          <label key={String(option.value)} className="flex items-center gap-3 rounded-xl border border-[#D9E0EA] px-4 py-3 text-sm font-medium">
            <input type="checkbox" checked={selected.includes(String(option.value))} onChange={() => onToggle(String(option.value))} /> {option.label}
          </label>
        ))}
      </div>
    </fieldset>
...
function RelationPicker({
  ...
  return (
    <fieldset className="rounded-xl border border-[#D9E0EA] p-4">
      <legend className="px-1 text-sm font-semibold text-[#344054]">{title}</legend>
      <p className="mt-1 text-sm text-[#667085]">{disabled ? 'Save the country first, then curate published records.' : description}</p>
      <div className="mt-3 max-h-56 space-y-2 overflow-y-auto">
        {options.length ? (
          options.map((option) => (
            <label key={option.id} className="flex items-center gap-3 rounded-lg px-2 py-2 text-sm">
              <input disabled={disabled} type="checkbox" checked={selected.includes(option.id)} onChange={() => onToggle(option.id)} />{' '}
              <span>
                {option.name}
          ...
          <p className="text-sm text-[#667085]">No eligible published records yet.</p>
        )}
      </div>
    </fieldset>
  );
}
function CountrySection({ index, row, media, onChange, onRemove }: { index: number; row: SectionRow; media: EditorialMedia[]; onChange: (patch: Partial<SectionRow>) => void; onRemove: () => void }) {
  return (
    <div className="rounded-2xl border border-[#E8ECF3] bg-[#FBFCFE] p-5">
      <div className="flex justify-between">
        <h4 className="font-semibold">Content section {index + 1}</h4>
        <button type="button" onClick={onRemove} className="text-sm font-semibold text-[#B42318]">
          Remove
        </button>
      </div>
      <div className="mt-4 grid gap-4 sm:grid-cols-2">
        <Select label="Section key" value={row.sectionKey} onChange={(value) => onChange({ sectionKey: value as SectionRow['sectionKey'] })} options={SECTION_KEYS.map((id) => ({ id, label: id }))} />
        <Select
          label="Section type"
        ...
        />
        <Input label="Eyebrow" value={row.eyebrow} onChange={(value) => onChange({ eyebrow: value })} />
        <Input label="Heading" value={row.heading} onChange={(value) => onChange({ heading: value })} />
        <Input label="Subheading" value={row.subheading} onChange={(value) => onChange({ subheading: value })} textarea span />
        <Input label="Display order" value={String(row.displayOrder)} onChange={(value) => onChange({ displayOrder: Number(value) || 0 })} type="number" />
        <MediaPickerDialog label="Primary media" value={row.primaryMediaId} media={media} onChange={(value) => onChange({ primaryMediaId: value })} />
        <MediaPickerDialog label="Secondary media" value={row.secondaryMediaId} media={media} onChange={(value) => onChange({ secondaryMediaId: value })} />
        <Input label="CTA label" value={row.ctaLabel} onChange={(value) => onChange({ ctaLabel: value })} />
        <Input label="CTA URL" value={row.ctaUrl} onChange={(value) => onChange({ ctaUrl: value })} />
      </div>
      <div className="mt-5">
        <TypedBodyEditor type={row.sectionType} value={row.body} onChange={(body) => onChange({ body })} variables={variablesForContext('country')} />
      </div>
    </div>
  );
}
