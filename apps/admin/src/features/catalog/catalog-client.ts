...
import type { CatalogEnvelope, CatalogListParams, CatalogMutationError, ContinentRecord, CountryRecord, CountryCurationOptions, CountryProfileBundle, IntakeOption, DirectoryRecord, PageMeta, SuggestionRecord, CountryEditorialBundle, EditorialCard, EditorialFaq, EditorialSection, EditorialMedia, SubjectRecord, SubSubjectRecord, MasterRecord, CourseRecord, CourseMappingRecord, CourseSectionRecord, CourseFaqRecord, CourseRelatedRecord, EditorialSeo } from './catalog.types';
...
async function request<T>(path: string, init: RequestInit = {}): Promise<{ data: T; meta: PageMeta | null }> {
  ...
  try {
    ...
    throw Object.assign(new Error('Catalog service is temporarily unavailable'), {
...
export function getCountryCurationOptions(id: string) {
  return request<CountryCurationOptions>(`/api/v1/admin/countries/${id}/curation-options`);
}
...
export function putCountryProfile(id: string, profile: 'cost' | 'work' | 'language' | 'intakes' | 'statistics', data: Record<string, unknown>) {
  return request<Record<string, unknown> | { intakes: Array<Record<string, unknown>> }>(`/api/v1/admin/countries/${id}/profiles/${profile}`, {
    method: 'PUT',
    body: JSON.stringify(data),
  });
}
...
export function getCountryEditorial(id: string) {
  return request<CountryEditorialBundle>(`/api/v1/admin/countries/${id}/editorial`);
}
...
export function listEditorialMedia(params: { q?: string; limit?: number } = {}) {
  const limit = params.limit === undefined ? undefined : Math.min(params.limit, MEDIA_OPTIONS_MAX_LIMIT);
  return request<EditorialMedia[]>(`/api/v1/admin/media-options${query({ ...params, limit })}`);
}
export function createEditorialSection(id: string, data: Record<string, unknown>) {
  return request<EditorialSection>(`/api/v1/admin/countries/${id}/content-sections`, { method: 'POST', body: JSON.stringify(data) });
}
export function updateEditorialSection(countryId: string, sectionId: string, data: Record<string, unknown>) {
  return request<EditorialSection>(`/api/v1/admin/countries/${countryId}/content-sections/${sectionId}`, { method: 'PATCH', body: JSON.stringify(data) });
}
export function deleteEditorialSection(countryId: string, sectionId: string, expectedUpdatedAt?: string) {
  return request<{ deleted: true }>(`/api/v1/admin/countries/${countryId}/content-sections/${sectionId}`, { method: 'DELETE', body: JSON.stringify({ expectedUpdatedAt }) });
}
...
export function updateCountryFaq(countryId: string, faqId: string, data: Record<string, unknown>) {
  return request<EditorialFaq>(`/api/v1/admin/countries/${countryId}/faqs/${faqId}`, { method: 'PATCH', body: JSON.stringify(data) });
}
export function deleteCountryFaq(countryId: string, faqId: string, expectedUpdatedAt?: string) {
  return request<{ deleted: true }>(`/api/v1/admin/countries/${countryId}/faqs/${faqId}`, { method: 'DELETE', body: JSON.stringify({ expectedUpdatedAt }) });
}
...
export function createConsultantCard(id: string, data: Record<string, unknown>) {
  return request<EditorialCard>(`/api/v1/admin/countries/${id}/consultant-cards`, { method: 'POST', body: JSON.stringify(data) });
}
export function updateConsultantCard(countryId: string, cardId: string, data: Record<string, unknown>) {
  return request<EditorialCard>(`/api/v1/admin/countries/${countryId}/consultant-cards/${cardId}`, { method: 'PATCH', body: JSON.stringify(data) });
}
export function deleteConsultantCard(countryId: string, cardId: string, expectedUpdatedAt?: string) {
  return request<{ deleted: true }>(`/api/v1/admin/countries/${countryId}/consultant-cards/${cardId}`, { method: 'DELETE', body: JSON.stringify({ expectedUpdatedAt }) });
}

export function listSubjects(params: CatalogListParams = {}, signal?: AbortSignal) {
...
export function listSubSubjects(subjectId: string, params: CatalogListParams = {}) {
  return request<SubSubjectRecord[]>(`/api/v1/admin/subjects/${subjectId}/sub-subjects${query(params)}`);
}
export function createSubSubject(subjectId: string, data: Record<string, unknown>) {
  return request<SubSubjectRecord>(`/api/v1/admin/subjects/${subjectId}/sub-subjects`, { method: 'POST', body: JSON.stringify(data) });
}
export function updateSubSubject(subjectId: string, id: string, data: Record<string, unknown>) {
  return request<SubSubjectRecord>(`/api/v1/admin/subjects/${subjectId}/sub-subjects/${id}`, { method: 'PATCH', body: JSON.stringify(data) });
}
export function publishSubSubject(subjectId: string, id: string, expectedUpdatedAt?: string) {
  return request<SubSubjectRecord>(`/api/v1/admin/subjects/${subjectId}/sub-subjects/${id}/publish`, { method: 'POST', body: JSON.stringify({ expectedUpdatedAt }) });
}
export function unpublishSubSubject(subjectId: string, id: string, expectedUpdatedAt?: string) {
  return request<SubSubjectRecord>(`/api/v1/admin/subjects/${subjectId}/sub-subjects/${id}/unpublish`, { method: 'POST', body: JSON.stringify({ expectedUpdatedAt }) });
}
export function deleteSubSubject(subjectId: string, id: string, expectedUpdatedAt?: string) {
  return request<{ deleted: true }>(`/api/v1/admin/subjects/${subjectId}/sub-subjects/${id}`, { method: 'DELETE', body: JSON.stringify({ expectedUpdatedAt }) });
}
...
export function replaceCourseModes(id: string, studyModeIds: string[], expectedUpdatedAt?: string) {
...
export function updateCourseMapping(courseId: string, mappingId: string, data: Record<string, unknown>) {
  return request<CourseMappingRecord>(`/api/v1/admin/courses/${courseId}/countries/${mappingId}`, { method: 'PATCH', body: JSON.stringify(data) });
}
export function deleteCourseMapping(courseId: string, mappingId: string, expectedUpdatedAt?: string) {
  return request<{ deleted: true }>(`/api/v1/admin/courses/${courseId}/countries/${mappingId}`, { method: 'DELETE', body: JSON.stringify({ expectedUpdatedAt }) });
}
export function listCourseIntakes(courseId: string, mappingId: string) {
  return request<Array<Record<string, unknown>>>(`/api/v1/admin/courses/${courseId}/countries/${mappingId}/intakes`);
}
export function replaceCourseIntakes(courseId: string, mappingId: string, intakes: Array<Record<string, unknown>>, expectedUpdatedAt?: string) {
  return request<Array<Record<string, unknown>>>(`/api/v1/admin/courses/${courseId}/countries/${mappingId}/intakes`, { method: 'PUT', body: JSON.stringify({ intakes, expectedUpdatedAt }) });
}
export function listCourseSections(id: string) {
  return request<CourseSectionRecord[]>(`/api/v1/admin/courses/${id}/content-sections`);
}
export function createCourseSection(id: string, data: Record<string, unknown>) {
  return request<CourseSectionRecord>(`/api/v1/admin/courses/${id}/content-sections`, { method: 'POST', body: JSON.stringify(data) });
}
export function updateCourseSection(courseId: string, sectionId: string, data: Record<string, unknown>) {
  return request<CourseSectionRecord>(`/api/v1/admin/courses/${courseId}/content-sections/${sectionId}`, { method: 'PATCH', body: JSON.stringify(data) });
}
export function deleteCourseSection(courseId: string, sectionId: string, expectedUpdatedAt?: string) {
  return request<{ deleted: true }>(`/api/v1/admin/courses/${courseId}/content-sections/${sectionId}`, { method: 'DELETE', body: JSON.stringify({ expectedUpdatedAt }) });
}
...
export function updateCourseFaq(courseId: string, faqId: string, data: Record<string, unknown>) {
  return request<CourseFaqRecord>(`/api/v1/admin/courses/${courseId}/faqs/${faqId}`, { method: 'PATCH', body: JSON.stringify(data) });
}
export function deleteCourseFaq(courseId: string, faqId: string, expectedUpdatedAt?: string) {
  return request<{ deleted: true }>(`/api/v1/admin/courses/${courseId}/faqs/${faqId}`, { method: 'DELETE', body: JSON.stringify({ expectedUpdatedAt }) });
}
export function listCourseRelated(id: string) {
  return request<CourseRelatedRecord[]>(`/api/v1/admin/courses/${id}/related`);
}
export function replaceCourseRelated(id: string, related: Array<Record<string, unknown>>, expectedUpdatedAt?: string) {
  return request<CourseRelatedRecord[]>(`/api/v1/admin/courses/${id}/related`, {
    method: 'PUT',
