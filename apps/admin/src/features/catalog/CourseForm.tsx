    ...
    <section aria-labelledby="course-form-heading" className="mx-auto max-w-[1100px]">
      ...
      <form onSubmit={submit} className="mt-8 space-y-7 rounded-2xl border border-[#E8ECF3] bg-white p-6 sm:p-8">
        <div className="grid gap-5 sm:grid-cols-3">
          <label className="text-sm font-semibold"><FieldLabel label="Subject" required help={commonFieldHelp.subject} /><select required className="mt-2 w-full rounded-xl border border-[#D9E0EA] px-3 py-3 font-normal" value={form.subjectId} onChange={(event) => set('subjectId', event.target.value)}><option value="">Select subject</option>{subjects.map((subject) => <option key={subject.id} value={subject.id}>{subject.name}</option>)}</select></label>
          <label className="text-sm font-semibold"><FieldLabel label="Course level" required helpKey="courses.courseLevelId" /><select required className="mt-2 w-full rounded-xl border border-[#D9E0EA] px-3 py-3 font-normal" value={form.courseLevelId} onChange={(event) => set('courseLevelId', event.target.value)}><option value="">Select level</option>{levels.map((level) => <option key={level.id} value={level.id}>{level.name}</option>)}</select></label>
          <label className="text-sm font-semibold"><FieldLabel label="Sub-Subject ID" helpKey="courses.subSubjectId" /><input className="mt-2 w-full rounded-xl border border-[#D9E0EA] px-3 py-3 font-normal" value={form.subSubjectId} onChange={(event) => set('subSubjectId', event.target.value)} placeholder="Optional UUID" /></label>
        </div>
        <div className="grid gap-5 sm:grid-cols-2">
          <label className="text-sm font-semibold"><FieldLabel label="Course name" required help={commonFieldHelp.name} /><input required maxLength={255} className="mt-2 w-full rounded-xl border border-[#D9E0EA] px-3 py-3 font-normal" value={form.name} onChange={(event) => set('name', event.target.value)} /></label>
          <label className="text-sm font-semibold"><FieldLabel label="Slug" help={commonFieldHelp.slug} /><input pattern="[a-z0-9]+(?:-[a-z0-9]+)*" className="mt-2 w-full rounded-xl border border-[#D9E0EA] px-3 py-3 font-normal" value={form.slug} onChange={(event) => set('slug', event.target.value)} placeholder="Optional; generated from course name" /></label>
        </div>
        <div className="grid gap-5 sm:grid-cols-3">
          <label className="text-sm font-semibold"><FieldLabel label="Short name" helpKey="courses.shortName" /><input className="mt-2 w-full rounded-xl border border-[#D9E0EA] px-3 py-3 font-normal" value={form.shortName} onChange={(event) => set('shortName', event.target.value)} /></label>
          <label className="text-sm font-semibold"><FieldLabel label="Qualification" helpKey="courses.qualificationName" /><input className="mt-2 w-full rounded-xl border border-[#D9E0EA] px-3 py-3 font-normal" value={form.qualificationName} onChange={(event) => set('qualificationName', event.target.value)} /></label>
          ...
          <label className="text-sm font-semibold"><FieldLabel label="Course code" helpKey="courses.courseCode" /><input className="mt-2 w-full rounded-xl border border-[#D9E0EA] px-3 py-3 font-normal" value={form.courseCode} onChange={(event) => set('courseCode', event.target.value)} /></label>
        </div>
        <label className="block text-sm font-semibold"><FieldLabel label="Short description" help={commonFieldHelp.shortDescription} /><textarea maxLength={1000} className="mt-2 min-h-24 w-full rounded-xl border border-[#D9E0EA] px-3 py-3 font-normal" value={form.shortDescription} onChange={(event) => set('shortDescription', event.target.value)} /></label>
        <label className="block text-sm font-semibold"><FieldLabel label="Overview" help={commonFieldHelp.overview} /><textarea maxLength={20000} className="mt-2 min-h-36 w-full rounded-xl border border-[#D9E0EA] px-3 py-3 font-normal" value={form.overview} onChange={(event) => set('overview', event.target.value)} /></label>
        <fieldset>
        ...
        <div className="grid gap-5 sm:grid-cols-4">
          <label className="text-sm font-semibold"><FieldLabel label="Duration min" helpKey="courses.durationMin" /><input inputMode="decimal" className="mt-2 w-full rounded-xl border border-[#D9E0EA] px-3 py-3 font-normal" value={form.durationMin} onChange={(event) => set('durationMin', event.target.value)} /></label>
          <label className="text-sm font-semibold"><FieldLabel label="Duration max" helpKey="courses.durationMax" /><input inputMode="decimal" className="mt-2 w-full rounded-xl border border-[#D9E0EA] px-3 py-3 font-normal" value={form.durationMax} onChange={(event) => set('durationMax', event.target.value)} /></label>
          <label className="text-sm font-semibold"><FieldLabel label="Unit" helpKey="courses.durationUnit" /><select className="mt-2 w-full rounded-xl border border-[#D9E0EA] px-3 py-3 font-normal" value={form.durationUnit} onChange={(event) => set('durationUnit', event.target.value)}><option value="YEARS">Years</option><option value="MONTHS">Months</option><option value="SEMESTERS">Semesters</option></select></label>
          <label className="text-sm font-semibold"><FieldLabel label="Credits" helpKey="courses.credits" /><input inputMode="decimal" className="mt-2 w-full rounded-xl border border-[#D9E0EA] px-3 py-3 font-normal" value={form.credits} onChange={(event) => set('credits', event.target.value)} /></label>
        </div>
        <div className="grid gap-5 sm:grid-cols-3">
          <label className="text-sm font-semibold"><FieldLabel label="Career summary" helpKey="courses.careerSummary" /><textarea className="mt-2 min-h-24 w-full rounded-xl border border-[#D9E0EA] px-3 py-3 font-normal" value={form.careerSummary} onChange={(event) => set('careerSummary', event.target.value)} /></label>
          <label className="text-sm font-semibold"><FieldLabel label="Popularity score" helpKey="courses.popularityScore" /><input inputMode="decimal" className="mt-2 w-full rounded-xl border border-[#D9E0EA] px-3 py-3 font-normal" value={form.popularityScore} onChange={(event) => set('popularityScore', event.target.value)} /></label>
          <label className="flex items-center gap-3 self-center text-sm font-semibold"><input type="checkbox" checked={form.isFeatured} onChange={(event) => set('isFeatured', event.target.checked)} /><FieldLabel label="Featured course" help={commonFieldHelp.featured} /></label>
        </div>
