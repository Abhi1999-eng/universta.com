...
type CoreState = {
  subjectId: string; subSubjectId: string; courseLevelId: string; name: string; shortName: string; qualificationName: string; slug: string; courseCode: string; shortDescription: string; overview: string; durationMin: string; durationMax: string; durationUnit: string; credits: string; careerSummary: string; popularityScore: string; isFeatured: boolean; displayOrder: string; featuredMediaId: string;
};
type IntakeDraft = { intakeId: string; applicationDeadline: string };
type MappingDraft = {
  id?: string; updatedAt?: string; countryId: string; availabilityStatus: string; status: string; indicativeTuitionMin: string; indicativeTuitionMax: string; currencyCode: string; tuitionPeriod: string; applicationFeeMin: string; applicationFeeMax: string; durationMinOverride: string; durationMaxOverride: string; durationUnitOverride: string; academicMinPercentage: string; academicMinCgpa: string; ieltsMinScore: string; pteMinScore: string; toeflMinScore: string; duolingoMinScore: string; workExperienceMonths: string; scholarshipAvailable: boolean; admissionRequirements: string; englishRequirements: string; applicationNotes: string; careerOpportunities: string; sourceReference: string; verifiedAt: string; isFeatured: boolean; displayOrder: string; intakes: IntakeDraft[];
};
type SectionDraft = { id?: string; updatedAt?: string; sectionKey: string; sectionType: string; heading: string; subheading: string; bodyText: string; mediaId: string; status: string; displayOrder: string };
type FaqDraft = { id?: string; updatedAt?: string; question: string; answer: string; status: string; displayOrder: string };

const coreBlank: CoreState = { subjectId: '', subSubjectId: '', courseLevelId: '', name: '', shortName: '', qualificationName: '', slug: '', courseCode: '', shortDescription: '', overview: '', durationMin: '', durationMax: '', durationUnit: 'YEARS', credits: '', careerSummary: '', popularityScore: '', isFeatured: false, displayOrder: '0', featuredMediaId: '' };
const mappingBlank = (): MappingDraft => ({ countryId: '', availabilityStatus: 'AVAILABLE', status: 'ACTIVE', indicativeTuitionMin: '', indicativeTuitionMax: '', currencyCode: '', tuitionPeriod: 'PER_YEAR', applicationFeeMin: '', applicationFeeMax: '', durationMinOverride: '', durationMaxOverride: '', durationUnitOverride: '', academicMinPercentage: '', academicMinCgpa: '', ieltsMinScore: '', pteMinScore: '', toeflMinScore: '', duolingoMinScore: '', workExperienceMonths: '', scholarshipAvailable: false, admissionRequirements: '', englishRequirements: '', applicationNotes: '', careerOpportunities: '', sourceReference: '', verifiedAt: '', isFeatured: false, displayOrder: '0', intakes: [] });
...
function mappingFromRecord(row: CourseMappingRecord): MappingDraft {
  return {
    id: row.id, updatedAt: row.updatedAt, countryId: row.country.id, availabilityStatus: row.availabilityStatus, status: row.status,
    indicativeTuitionMin: row.indicativeTuitionMin ?? '', indicativeTuitionMax: row.indicativeTuitionMax ?? '', currencyCode: row.currencyCode ?? '', tuitionPeriod: row.tuitionPeriod,
    applicationFeeMin: row.applicationFeeMin ?? '', applicationFeeMax: row.applicationFeeMax ?? '', durationMinOverride: row.durationMinOverride ?? '', durationMaxOverride: row.durationMaxOverride ?? '', durationUnitOverride: row.durationUnitOverride ?? '',
    academicMinPercentage: row.academicMinPercentage ?? '', academicMinCgpa: row.academicMinCgpa ?? '', ieltsMinScore: row.ieltsMinScore ?? '', pteMinScore: row.pteMinScore ?? '', toeflMinScore: row.toeflMinScore ?? '', duolingoMinScore: row.duolingoMinScore ?? '', workExperienceMonths: row.workExperienceMonths === null ? '' : String(row.workExperienceMonths),
    ...
    intakes: (row.intakes ?? []).map((raw) => { const item = raw as Record<string, unknown>; const nested = item.intake as Record<string, unknown> | undefined; return { intakeId: String(item.intakeId ?? nested?.id ?? ''), applicationDeadline: item.applicationDeadline ? String(item.applicationDeadline).slice(0, 10) : item.deadline ? String(item.deadline).slice(0, 10) : '' }; }).filter((item) => item.intakeId),
  };
}
function sectionBodyText(body: Record<string, unknown> | null) {
  ...
  const values: string[] = [];
  const collect = (value: unknown) => { if (typeof value === 'string' || typeof value === 'number') values.push(String(value)); else if (Array.isArray(value)) value.forEach(collect); else if (value && typeof value === 'object') Object.values(value as Record<string, unknown>).forEach(collect); };
  collect(body); return values.join('\n');
}
function sectionFromRecord(row: CourseSectionRecord): SectionDraft { return { id: row.id, updatedAt: row.updatedAt, sectionKey: row.sectionKey, sectionType: row.sectionType, heading: row.heading ?? '', subheading: row.subheading ?? '', bodyText: sectionBodyText(row.bodyJson), mediaId: row.media?.id ?? '', status: row.status, displayOrder: String(row.displayOrder) }; }
function faqFromRecord(row: CourseFaqRecord): FaqDraft { return { id: row.id, updatedAt: row.updatedAt, question: row.question, answer: row.answer, status: row.status, displayOrder: String(row.displayOrder) }; }
function bodyJson(text: string, type: string) { const paragraphs = text.split(/\n\s*\n/).map((x) => x.trim()).filter(Boolean); const lines = text.split(/\n+/).map((x) => x.trim()).filter(Boolean); if (type === 'CHECKLIST') return { paragraphs, items: lines }; if (type === 'STEPS') return { paragraphs, steps: lines.map((value, index) => ({ order: index + 1, text: value })) }; return { paragraphs }; }
...
export function CourseForm({ id }: { id?: string }) {
    ...
    const load = async () => {
      try {
        const base = await Promise.all([
          listSubjects({ limit: 100 }), listCourseLevels({ status: 'ACTIVE', limit: 100 }), listStudyModes({ status: 'ACTIVE', limit: 100 }), listEditorialMedia({ limit: 50 }), listCountries({ status: 'PUBLISHED', limit: 100 }), listIntakeOptions(), listAdminCourses({ limit: 100 }),
        ]);
        if (!active) return;
        setSubjects(base[0].data); setLevels(base[1].data); setModes(base[2].data); setMedia(base[3].data); setCountries(base[4].data); setIntakeOptions(base[5].data); setAllCourses(base[6].data);
        if (!id) return;
        const [courseResult, mappingResult, sectionResult, faqResult, relatedResult, seoResult] = await Promise.all([
          getAdminCourse(id), listCourseMappings(id), listCourseSections(id), listCourseFaqs(id), listCourseRelated(id), getCourseSeo(id),
        ]);
        if (!active) return;
        const course = courseResult.data;
        setRecord(course);
        setCore({ subjectId: course.subject.id, subSubjectId: course.subSubject?.id ?? '', courseLevelId: course.courseLevel.id, name: course.name, shortName: course.shortName ?? '', qualificationName: course.qualificationName ?? '', slug: course.slug, courseCode: course.courseCode ?? '', shortDescription: course.shortDescription ?? '', overview: course.overview ?? '', durationMin: course.durationMin ?? '', durationMax: course.durationMax ?? '', durationUnit: course.durationUnit ?? 'YEARS', credits: course.credits ?? '', careerSummary: course.careerSummary ?? '', popularityScore: course.popularityScore ?? '', isFeatured: course.featured, displayOrder: String(course.displayOrder), featuredMediaId: course.featuredMedia?.id ?? '' });
        setSelectedModes(course.studyModes.map((mode) => mode.id));
        setMappings(mappingResult.data.map(mappingFromRecord)); setSections(sectionResult.data.map(sectionFromRecord)); setFaqs(faqResult.data.map(faqFromRecord)); setRelatedIds(relatedResult.data.map((row: CourseRelatedRecord) => row.relatedCourse.id)); setExistingSeo(seoResult.data); setSeo(seoFromRecord(seoResult.data));
        const subResult = await listSubSubjects(course.subject.id, { limit: 100 }); if (active) setSpecializations(subResult.data);
      } catch (cause: unknown) { if (active) setError(cause instanceof Error ? cause.message : 'Unable to load course editor'); }
  ...
  const setCoreField = <K extends keyof CoreState>(key: K, value: CoreState[K]) => { setCore((current) => ({ ...current, [key]: value })); setDirty(true); setIssues([]); };
  const updateMapping = (index: number, patch: Partial<MappingDraft>) => { setMappings((rows) => rows.map((row, i) => i === index ? { ...row, ...patch } : row)); setDirty(true); };
  const updateSection = (index: number, patch: Partial<SectionDraft>) => { setSections((rows) => rows.map((row, i) => i === index ? { ...row, ...patch } : row)); setDirty(true); };
  const updateFaq = (index: number, patch: Partial<FaqDraft>) => { setFaqs((rows) => rows.map((row, i) => i === index ? { ...row, ...patch } : row)); setDirty(true); };
  const removeMapping = (index: number) => { const row = mappings[index]; if (row.id) setRemovedMappings((old) => [...old, row]); setMappings((rows) => rows.filter((_, i) => i !== index)); setDirty(true); };
  ...
  const toggleMode = (modeId: string) => { setSelectedModes((ids) => ids.includes(modeId) ? ids.filter((idValue) => idValue !== modeId) : [...ids, modeId]); setDirty(true); };
  const toggleRelated = (courseId: string) => { setRelatedIds((ids) => ids.includes(courseId) ? ids.filter((idValue) => idValue !== courseId) : [...ids, courseId]); setDirty(true); };
  ...
  function validate(intent: Intent) {
    ...
    for (const [value, label] of [[core.durationMin, 'Minimum duration'], [core.durationMax, 'Maximum duration'], [core.credits, 'Credits'], [core.popularityScore, 'Popularity score']] as const) if (value && !decimalPattern.test(value)) next.push(`${label} must be a non-negative number with at most 2 decimal places.`);
    ...
    activeSections.forEach((row, index) => { if (!row.sectionKey.trim()) next.push(`Content section ${index + 1}: section key is required.`); if (!/^\d+$/.test(row.displayOrder)) next.push(`Content section ${index + 1}: display order must be a whole number.`); });
    activeFaqs.forEach((row, index) => { if (!row.question.trim() || !row.answer.trim()) next.push(`FAQ ${index + 1}: both question and answer are required.`); });
    ...
    if (intent === 'publish') {
      const subject = subjects.find((row) => row.id === core.subjectId); if (subject && subject.status !== 'PUBLISHED') next.push('Selected Subject must be published before this Course.');
      const specialization = specializations.find((row) => row.id === core.subSubjectId); if (specialization && specialization.status !== 'PUBLISHED') next.push('Selected Specialization must be published before this Course.');
      if (selectedModes.length === 0) next.push('Select at least one Study mode before publishing.');
      const verified = activeMappings.some((row) => ['AVAILABLE', 'LIMITED'].includes(row.availabilityStatus) && /^https:\/\//i.test(row.sourceReference.trim()) && Boolean(row.verifiedAt)); if (!verified) next.push('Publish requires at least one Available/Limited country with source URL and verified date.');
    }
    setIssues(next); return next.length === 0;
  }

  function mappingPayload(row: MappingDraft) {
    const data: Record<string, unknown> = { countryId: row.countryId, availabilityStatus: row.availabilityStatus, status: row.status, tuitionPeriod: row.tuitionPeriod, scholarshipAvailable: row.scholarshipAvailable, isFeatured: row.isFeatured, displayOrder: Number(row.displayOrder) || 0 };
    ...
    stringKeys.forEach((key) => { const value = row[key].trim(); if (value) data[key] = key === 'currencyCode' ? value.toUpperCase() : value; });
  ...
  async function syncMappings(courseId: string) {
    ...
    for (const row of activeMappings) {
      ...
      await replaceCourseIntakes(courseId, current.id, row.intakes.filter((item) => item.intakeId).map((item) => ({ intakeId: item.intakeId, ...(item.applicationDeadline ? { applicationDeadline: item.applicationDeadline } : {}), status: 'ACTIVE' })), current.updatedAt);
      saved.push({ ...mappingFromRecord(current), intakes: row.intakes });
    }
    setMappings(saved); setRemovedMappings([]);
  }
  async function syncSections(courseId: string) {
    ...
    for (const row of activeSections) {
      const payload = { sectionKey: row.sectionKey.trim(), sectionType: row.sectionType, heading: optional(row.heading), subheading: optional(row.subheading), bodyJson: bodyJson(row.bodyText, row.sectionType), ...(row.mediaId ? { mediaId: row.mediaId } : {}), status: row.status, displayOrder: Number(row.displayOrder) || 0, ...(row.updatedAt ? { expectedUpdatedAt: row.updatedAt } : {}) };
      const result = row.id ? await updateCourseSection(courseId, row.id, payload) : await createCourseSection(courseId, payload); saved.push(sectionFromRecord(result.data));
    }
    setSections(saved); setRemovedSections([]);
  }
  async function syncFaqs(courseId: string) {
    ...
    for (const row of activeFaqs) {
      const payload = { question: row.question.trim(), answer: row.answer.trim(), status: row.status, displayOrder: Number(row.displayOrder) || 0, ...(row.updatedAt ? { expectedUpdatedAt: row.updatedAt } : {}) };
      const result = row.id ? await updateCourseFaq(courseId, row.id, payload) : await createCourseFaq(courseId, payload); saved.push(faqFromRecord(result.data));
    }
    setFaqs(saved); setRemovedFaqs([]);
  }

  async function submit(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault(); if (saving) return;
    const submitter = (event.nativeEvent as SubmitEvent).submitter as HTMLButtonElement | null; const intent: Intent = submitter?.value === 'publish' ? 'publish' : 'draft';
    if (!validate(intent)) return;
    setSaving(true); setSavingIntent(intent); setError('');
    try {
      const corePayload = { subjectId: core.subjectId, subSubjectId: optional(core.subSubjectId), courseLevelId: core.courseLevelId, name: core.name.trim(), shortName: optional(core.shortName), qualificationName: optional(core.qualificationName), slug: optional(core.slug) ?? slugify(core.name), courseCode: optional(core.courseCode), shortDescription: optional(core.shortDescription), overview: optional(core.overview), durationMin: optional(core.durationMin), durationMax: optional(core.durationMax), durationUnit: optional(core.durationUnit), credits: optional(core.credits), careerSummary: optional(core.careerSummary), popularityScore: optional(core.popularityScore), isFeatured: core.isFeatured, displayOrder: Number(core.displayOrder) || 0, ...(core.featuredMediaId ? { featuredMediaId: core.featuredMediaId } : {}), ...(record ? { expectedUpdatedAt: record.updatedAt } : {}) };
      ...
      await syncMappings(saved.id); await syncSections(saved.id); await syncFaqs(saved.id);
      ...
      if (hasSeo(seo)) { const result = await saveCourseSeo(saved.id, seoPayload(seo)); setExistingSeo(result.data); } else if (existingSeo) { await deleteCourseSeo(saved.id, existingSeo.updatedAt); setExistingSeo(null); }
      const refreshed = (await getAdminCourse(saved.id)).data;
      const finalRecord = intent === 'publish' ? (refreshed.status === 'PUBLISHED' ? refreshed : (await publishCourse(refreshed.id, refreshed.updatedAt)).data) : (refreshed.status === 'PUBLISHED' ? (await unpublishCourse(refreshed.id, refreshed.updatedAt)).data : refreshed);
      setRecord(finalRecord); setDirty(false); if (!id) router.replace(`/courses/${finalRecord.id}`); router.refresh();
    } catch (cause: unknown) { const typed = cause as Partial<CatalogMutationError>; setError(typed.message ?? (cause instanceof Error ? cause.message : 'Unable to save course')); }
  ...
  return (
    <section className="mx-auto max-w-[1180px]" aria-labelledby="course-form-heading">
      ...
      <div className="mt-8 flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-[#828B9B]">Unified course editor</p><h2 id="course-form-heading" className="mt-2 text-3xl font-semibold">{record ? 'Edit course' : 'Create course'}</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-[#667085]">Core data, availability, content, FAQs, related courses and SEO load on one page and save together.</p></div><span className="rounded-full border border-[#D9E0EA] px-4 py-2 text-xs font-bold uppercase tracking-[0.12em] text-[#667085]">{record?.status ?? 'DRAFT'}</span></div>
      ...
      <form onSubmit={submit} className="mt-8 space-y-6">
        ...
        <EditorCard eyebrow="Delivery" title="Study details" description="Duration, credits, career summary and supported study modes.">
          <div className="grid gap-5 sm:grid-cols-3"><Input label="Duration minimum" value={core.durationMin} onChange={(value) => setCoreField('durationMin', value)} /><Input label="Duration maximum" value={core.durationMax} onChange={(value) => setCoreField('durationMax', value)} /><Select label="Duration unit" value={core.durationUnit} onChange={(value) => setCoreField('durationUnit', value)} options={[{ id: 'MONTHS', label: 'Months' }, { id: 'YEARS', label: 'Years' }]} /><Input label="Credits" value={core.credits} onChange={(value) => setCoreField('credits', value)} /><Input label="Popularity score" value={core.popularityScore} onChange={(value) => setCoreField('popularityScore', value)} /><Input label="Career summary" value={core.careerSummary} onChange={(value) => setCoreField('careerSummary', value)} textarea span rows={4} /></div>
          <div className="mt-5"><p className="text-sm font-semibold">Study modes</p><div className="mt-3 flex flex-wrap gap-3">{modes.map((mode) => <label key={mode.id} className="flex items-center gap-2 rounded-xl border border-[#D9E0EA] px-4 py-3 text-sm font-semibold"><input type="checkbox" checked={selectedModes.includes(mode.id)} onChange={() => toggleMode(mode.id)} /> {mode.name}</label>)}</div></div>
        </EditorCard>

        <EditorCard eyebrow="Destinations" title="Country availability" description="All country mappings are part of the course form. Add/remove rows here; top Save Draft / Publish persists them.">
          <div className="flex justify-end"><button type="button" onClick={() => { setMappings((rows) => [...rows, mappingBlank()]); setDirty(true); }} className="rounded-xl border border-[#1657CF] px-4 py-2 text-sm font-semibold text-[#1657CF]">+ Add country</button></div>
          <div className="mt-5 space-y-5">{mappings.length === 0 ? <Empty text="No country availability added yet." /> : mappings.map((row, index) => <AvailabilityRow key={row.id ?? `mapping-${index}`} index={index} row={row} countries={countries} intakes={intakeOptions} onChange={(patch) => updateMapping(index, patch)} onRemove={() => removeMapping(index)} />)}</div>
        </EditorCard>

        <EditorCard eyebrow="Editorial" title="Content sections" description="Build the course page content here. No separate content save button.">
          <div className="flex justify-end"><button type="button" onClick={() => { setSections((rows) => [...rows, sectionBlank()]); setDirty(true); }} className="rounded-xl border border-[#1657CF] px-4 py-2 text-sm font-semibold text-[#1657CF]">+ Add section</button></div>
          <div className="mt-5 space-y-5">{sections.length === 0 ? <Empty text="No content sections yet." /> : sections.map((row, index) => <ContentRow key={row.id ?? `section-${index}`} index={index} row={row} media={media} onChange={(patch) => updateSection(index, patch)} onRemove={() => removeSection(index)} />)}</div>
        </EditorCard>

        <EditorCard eyebrow="Questions" title="FAQs" description="FAQs save with the course, not independently.">
          <div className="flex justify-end"><button type="button" onClick={() => { setFaqs((rows) => [...rows, faqBlank()]); setDirty(true); }} className="rounded-xl border border-[#1657CF] px-4 py-2 text-sm font-semibold text-[#1657CF]">+ Add FAQ</button></div>
          <div className="mt-5 space-y-4">{faqs.length === 0 ? <Empty text="No FAQs yet." /> : faqs.map((row, index) => <div key={row.id ?? `faq-${index}`} className="rounded-2xl border border-[#E8ECF3] bg-[#FBFCFE] p-5"><div className="flex justify-between gap-3"><h4 className="font-semibold">FAQ {index + 1}</h4><button type="button" onClick={() => removeFaq(index)} className="text-sm font-semibold text-[#B42318]">Remove</button></div><div className="mt-4 grid gap-4 sm:grid-cols-2"><Input label="Question" value={row.question} onChange={(value) => updateFaq(index, { question: value })} span /><Input label="Answer" value={row.answer} onChange={(value) => updateFaq(index, { answer: value })} textarea span rows={4} /><Select label="Status" value={row.status} onChange={(value) => updateFaq(index, { status: value })} options={[{ id: 'ACTIVE', label: 'Active' }, { id: 'INACTIVE', label: 'Inactive' }]} /><Input label="Display order" value={row.displayOrder} onChange={(value) => updateFaq(index, { displayOrder: value })} type="number" /></div></div>)}</div>
        </EditorCard>
