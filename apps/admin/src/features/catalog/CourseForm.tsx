...
function clean(form: FormState) {
  ...
  for (const key of [
    ...
    if (form[key]) result[key] = form[key];
  }
  return result;
}
...
export function CourseForm({ id }: { id?: string }) {
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      .then(
        ([subjectResult, levelResult, modeResult, mediaResult, courseResult]) => {
          if (cancelled) return;
      ...
      .catch((cause: unknown) =>
        setError(
          cause instanceof Error ? cause.message : 'Unable to load course options',
        ),
      )
  ...
  return (
    ...
    >
      <Link href="/courses" className="text-sm font-semibold text-[#1657CF]">
        ← Courses
      </Link>
      <p className="mt-8 text-xs font-bold uppercase tracking-[0.16em] text-[#828B9B]">
        Course editor
      </p>
      <h2 id="course-form-heading" className="mt-2 text-3xl font-semibold">
        {record ? 'Edit course' : 'Create course'}
      </h2>
      {error ? (
        <div className="mt-5">
          <CatalogError message={error} onRetry={() => setError('')} />
        </div>
      ) : null}
      <form
        onSubmit={submit}
        className="mt-8 space-y-7 rounded-2xl border border-[#E8ECF3] bg-white p-6 sm:p-8"
      >
        <div className="grid gap-5 sm:grid-cols-3">
          <div className="text-sm font-semibold">
            <FieldLabel
              ...
              required
              help={commonFieldHelp.subject}
            />
            <select
              id="course-subject"
              required
              className="mt-2 w-full rounded-xl border border-[#D9E0EA] px-3 py-3 font-normal"
          ...
          <div className="text-sm font-semibold">
            <FieldLabel
              ...
              htmlFor="course-specialization"
              helpKey="courses.subSubjectId"
            />
            <select
              id="course-specialization"
              className="mt-2 w-full rounded-xl border border-[#D9E0EA] px-3 py-3 font-normal disabled:bg-[#F7F9FC] disabled:text-[#98A2B3]"
              ...
              <option value="">
                {!form.subjectId
                  ...
                  : loadingSpecializations
                    ...
                    : 'No specialization / General course'}
              </option>
            ...
            {form.subjectId && !loadingSpecializations ? (
              <p className="mt-2 text-xs font-normal text-[#667085]">
                {specializations.length
                  ? 'Only specializations created under the selected Subject are shown.'
                  : 'No specializations have been created under this Subject yet.'}
              </p>
            ) : null}
          </div>
        </div>
        ...
        <div className="grid gap-5 sm:grid-cols-2">
          ...
          <div className="text-sm font-semibold">
            ...
            <input
              ...
              placeholder="Optional; generated from course name"
            />
          </div>
        </div>
        ...
        <div className="grid gap-5 sm:grid-cols-3">
          <div className="text-sm font-semibold">
            <FieldLabel
              label="Short name"
              htmlFor="course-short-name"
              helpKey="courses.shortName"
            />
            <input
          ...
          <div className="text-sm font-semibold">
            <FieldLabel
              label="Qualification"
              htmlFor="course-qualification"
              helpKey="courses.qualificationName"
            />
            <input
              id="course-qualification"
              className="mt-2 w-full rounded-xl border border-[#D9E0EA] px-3 py-3 font-normal"
              value={form.qualificationName}
              onChange={(event) => set('qualificationName', event.target.value)}
            />
          </div>
          <MediaPickerDialog
            label="Featured course media"
            value={form.featuredMediaId}
            media={media}
            onChange={(value) => set('featuredMediaId', value)}
          />
          <div className="text-sm font-semibold">
            <FieldLabel
              label="Course code"
              htmlFor="course-code"
              helpKey="courses.courseCode"
            />
            <input
              id="course-code"
              className="mt-2 w-full rounded-xl border border-[#D9E0EA] px-3 py-3 font-normal"
        ...
        <div className="block text-sm font-semibold">
          ...
          <textarea
            ...
            maxLength={1000}
            className="mt-2 min-h-24 w-full rounded-xl border border-[#D9E0EA] px-3 py-3 font-normal"
        ...
        <div className="block text-sm font-semibold">
          ...
          <textarea
            id="course-overview"
            maxLength={20000}
            className="mt-2 min-h-36 w-full rounded-xl border border-[#D9E0EA] px-3 py-3 font-normal"
            value={form.overview}
        ...
        <fieldset>
          {/* <legend> must stay a direct fieldset child with the icon as its
              sibling (not wrapped together in another element) — either
              wrapping breaks the fieldset's own accessible name entirely,
              not just when an icon is present. */}
          <legend className="flex items-center text-sm font-semibold">
            Study modes
          </legend>
          {studyModesHelp ? (
            <FieldHelpIcon fieldLabel="Study modes" help={studyModesHelp} />
          ) : null}
          <div className="mt-3 flex flex-wrap gap-3">
            {modes.map((mode) => (
              <label
                key={mode.id}
                className="flex items-center gap-2 rounded-lg border border-[#E8ECF3] px-3 py-2 text-sm"
              >
                <input
                  type="checkbox"
        ...
        <div className="grid gap-5 sm:grid-cols-4">
          <div className="text-sm font-semibold">
            <FieldLabel
              label="Duration min"
              htmlFor="course-duration-min"
              helpKey="courses.durationMin"
            />
            <input
              id="course-duration-min"
              inputMode="decimal"
              className="mt-2 w-full rounded-xl border border-[#D9E0EA] px-3 py-3 font-normal"
          ...
          <div className="text-sm font-semibold">
            <FieldLabel
              label="Duration max"
              htmlFor="course-duration-max"
              helpKey="courses.durationMax"
            />
            <input
              id="course-duration-max"
              inputMode="decimal"
              className="mt-2 w-full rounded-xl border border-[#D9E0EA] px-3 py-3 font-normal"
          ...
          <div className="text-sm font-semibold">
            <FieldLabel
              label="Unit"
              htmlFor="course-duration-unit"
              helpKey="courses.durationUnit"
            />
            <select
              id="course-duration-unit"
              className="mt-2 w-full rounded-xl border border-[#D9E0EA] px-3 py-3 font-normal"
              value={form.durationUnit}
              onChange={(event) => set('durationUnit', event.target.value)}
            >
        ...
        <div className="grid gap-5 sm:grid-cols-3">
          <div className="text-sm font-semibold">
            <FieldLabel
              ...
              htmlFor="course-career-summary"
              helpKey="courses.careerSummary"
            />
            <textarea
              id="course-career-summary"
              className="mt-2 min-h-24 w-full rounded-xl border border-[#D9E0EA] px-3 py-3 font-normal"
          ...
          <div className="text-sm font-semibold">
            <FieldLabel
              ...
              htmlFor="course-popularity-score"
              helpKey="courses.popularityScore"
            />
            <input
              id="course-popularity-score"
              inputMode="decimal"
              className="mt-2 w-full rounded-xl border border-[#D9E0EA] px-3 py-3 font-normal"
      ...
      {record ? (
        ...
        <p className="mt-5 text-sm text-[#667085]">
          Save the course core to open its editorial workspace for availability,
          content, FAQs, SEO, and related courses.
        </p>
      )}
    </section>
  );
