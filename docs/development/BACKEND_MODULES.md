...
| subjects | Subject and Sub-Subject catalog | `subjects`, `sub_subjects` | `GET /api/v1/subjects`, `/:slug` | subject/sub-subject CRUD, publish, SEO | slug/content/media validation, catalog permission, soft delete, audit; media/users |
| course-levels | Level catalog | `course_levels` | `GET /api/v1/course-levels` | level CRUD | code/order/dependency validation, catalog permission, audit |
| study-modes | Mode catalog | `study_modes` | `GET /api/v1/study-modes` | mode CRUD | code/order/dependency validation, catalog permission, audit |
| courses | Course catalog and country mappings | `courses`, study modes, country courses/intakes, content, FAQs, related | `GET /api/v1/courses`, suggestions, `/:slug` | course/mapping/intake/content/FAQ/related/SEO CRUD | slug/content/source/publication/soft-delete rules, catalog permission, audit; subjects/levels/countries/media |
